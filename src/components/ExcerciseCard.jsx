...
export const ExcerciseCard = ({ exercise, index }) => {
  const [setsCompleted, setSetsComplated] = useState(0)

  const handleSetIncrement = () => {
    setSetsComplated((setsCompleted + 1) % 6)
  }
  ...
  return (
    <div className='p-4 rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-wrap'>
      ...
      <div className='flex flex-col'>
        <h3 className='text-slate-400 text-sm'>Mucle Groups</h3>
        ...
        {['reps', 'rest', 'tempo'].map((info, index) => {
          return(
            <div key={index} className='flex flex-col p-2 rounded border-[1.5px] border-solid border-slate-900 w-full'>
              <h3 className='capitaliz text-slate-400 text-sm'>{info === 'reps' ? `${exercise.unit}` : info}</h3>
              <p className='font-medium'>{exercise[info]}</p>
            </div>
          )
        })}
        <button onClick={handleSetIncrement} className='flex flex-col p-2 rounded border-[1.5px] duration-200 border-solid border-blue-900 hover:border-blue-600 w-full'>Sets Completed<h3 className='text-slate-400 text-sm capitalize'></h3><p className='font-medium'>{setsCompleted}/5</p></button>
      </div>
    </div>
  )
}
